...
const MetricCard = ({ label, value, unit, color }) => (
  <div style={{
    ...
    <div style={{ color: "#aaa", fontSize: "0.85rem", marginBottom: "0.4rem" }}>{label}</div>
...
const ThirdsCard = ({ thirds }) => {
  ...
  const bars = [
    { label: "Upper Third", value: thirds.upperThird, ideal: 33.3, color: "#00d4ff" },
    { label: "Middle Third", value: thirds.middleThird, ideal: 33.3, color: "#ff6bff" },
    { label: "Lower Third", value: thirds.lowerThird, ideal: 33.3, color: "#ffcc00" },
  ];
  return (
    <div style={{ background: "#1a1a2e", borderRadius: "12px", padding: "1.5rem", border: "1px solid #00d4ff20" }}>
      ...
      {bars.map(bar => (
        <div key={bar.label} style={{ marginBottom: "0.8rem" }}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "0.3rem" }}>
            <span style={{ color: "#ccc", fontSize: "0.85rem" }}>{bar.label}</span>
            <span style={{ color: bar.color, fontSize: "0.85rem", fontWeight: "bold" }}>
              {bar.value}%
              <span style={{ color: abs(bar.value - bar.ideal) < 4 ? "#00ff88" : "#ff4444", marginLeft: "6px", fontSize: "0.75rem" }}>
                {abs(bar.value - bar.ideal) < 4 ? "✓" : `${bar.value > bar.ideal ? "+" : ""}${(bar.value - bar.ideal).toFixed(1)}%`}
              </span>
            </span>
          </div>
          <div style={{ background: "#0a0a1a", borderRadius: "4px", height: "8px", position: "relative" }}>
            <div style={{
      ...
      <div style={{ marginTop: "1rem", padding: "0.5rem 1rem", borderRadius: "6px", textAlign: "center",
        background: thirds.assessment === "Balanced" ? "#00ff8820" : "#ff444420",
        color: thirds.assessment === "Balanced" ? "#00ff88" : "#ff4444",
        fontSize: "0.85rem", fontWeight: "bold"
      }}>
        {thirds.assessment === "Balanced" ? "✅ Balanced Thirds" : "⚠️ Imbalanced Thirds"}
      </div>
      <div style={{ marginTop: "0.8rem", color: "#aaa", fontSize: "0.8rem" }}>
        Lower third — Upper lip: {thirds.lowerThirdUpperLip}% / Lower lip + chin: {thirds.lowerThirdLowerLip}%
        <span style={{ color: "#555", marginLeft: "6px" }}>(ideal 33% / 67%)</span>
      </div>
    </div>
  );
};
...
const FifthsCard = ({ fifths }) => {
  ...
  return (
    <div style={{ background: "#1a1a2e", borderRadius: "12px", padding: "1.5rem", border: "1px solid #ff6bff20" }}>
      <div style={{ color: "#ff6bff", fontWeight: "bold", marginBottom: "1rem" }}>
        📐 Vertical Fifths
      </div>
      <div style={{ color: "#aaa", fontSize: "0.8rem", marginBottom: "1rem" }}>
        Ideal: each fifth = 20%
      ...
      {bars.map(bar => (
        <div key={bar.label} style={{ display: "flex", justifyContent: "space-between", marginBottom: "0.4rem" }}>
          <span style={{ color: "#ccc", fontSize: "0.82rem" }}>{bar.label}</span>
          <span style={{ color: bar.color, fontSize: "0.82rem", fontWeight: "bold" }}>{bar.value}%</span>
        </div>
      ))}
      <div style={{ marginTop: "0.8rem", color: "#aaa", fontSize: "0.8rem" }}>
        Nose/Eye width ratio: <span style={{ color: fifths.noseEyeRatio >= 90 && fifths.noseEyeRatio <= 110 ? "#00ff88" : "#ffcc00", fontWeight: "bold" }}>
          {fifths.noseEyeRatio}%
        </span>
        <span style={{ color: "#555", marginLeft: "6px" }}>(ideal ~100%)</span>
      </div>
      <div style={{ marginTop: "0.8rem", padding: "0.5rem 1rem", borderRadius: "6px", textAlign: "center",
        background: fifths.assessment === "Balanced" ? "#00ff8820" : "#ff444420",
        color: fifths.assessment === "Balanced" ? "#00ff88" : "#ff4444",
        fontSize: "0.85rem", fontWeight: "bold"
      }}>
        {fifths.assessment === "Balanced" ? "✅ Balanced Fifths" : "⚠️ Asymmetric Fifths"}
      </div>
    </div>
  );
};
...
export default function AnalysisPage() {
  ...
  const handleCanvasMouseUp = async () => {
    ...
    try {
      const res = await axios.post(`${BACKEND_URL}/api/recalculate`, { landmarks });
      if (res.data.metrics) {
  ...
  const scoreColor = score >= 85 ? "#00ff88" : score >= 70 ? "#ffcc00" : "#ff4444";
  ...
  return (
    <div style={{
      ...
      <h2 style={{ textAlign: "center", color: "#00d4ff", marginBottom: "2rem" }}>
      ...
      <div style={{ maxWidth: "1000px", margin: "0 auto" }}>
        ...
        <label style={{
          ...
          <div style={{ color: "#555", fontSize: "0.8rem", marginTop: "0.3rem" }}>
        ...
        {/* Loading */}
        {loading && (
          <div style={{ textAlign: "center", color: "#00d4ff", padding: "2rem" }}>
        ...
        {result && (
          <>
            ...
            <div style={{ textAlign: "center", marginBottom: "2rem" }}>
              <div style={{ color: "#aaa", fontSize: "0.9rem" }}>Overall Symmetry Score</div>
              <div style={{ fontSize: "4rem", fontWeight: "bold", color: scoreColor }}>
                {result.metrics.symmetryScore}%
              </div>
              <div style={{ color: "#555", fontSize: "0.8rem" }}>
                {score >= 85 ? "Excellent symmetry" : score >= 70 ? "Moderate asymmetry" : "Significant asymmetry"}
              </div>
            </div>
            ...
            <div style={{
              ...
              <MetricCard label="Eye Canting" value={result.metrics.eyeCanting} unit="°" color="#00d4ff" />
              <MetricCard label="Lip Canting" value={result.metrics.lipCanting} unit="°" color="#ff6bff" />
              <MetricCard label="Chin Deviation" value={result.metrics.chinDeviation} unit="%" color="#ffcc00" />
              <MetricCard label="Nose Deviation" value={result.metrics.noseDeviation} unit="%" color="#00ff88" />
            </div>
...
<div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem", marginBottom: "2rem" }}>
  ...
  <FifthsCard fifths={result.metrics.verticalFifths} />
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }}>
              <div>
                <div style={{ color: "#aaa", marginBottom: "0.5rem", fontSize: "0.85rem" }}>
                  Original Photo
                </div>
                <img
                  src={originalImage}
              ...
              <div>
                <div style={{ color: "#aaa", marginBottom: "0.5rem", fontSize: "0.85rem" }}>
                  Landmark Analysis — <span style={{ color: "#00d4ff" }}>drag points to correct</span>
                </div>
